...
class ActivityUtils
{
    ...
    public ActivityUtils(Config config, Instrumentation inst, Activity activity,
            Sleeper sleeper)
    {
        this.config = config;
        this.inst = inst;
    ...
    public ArrayList<Activity> getAllOpenedActivities()
    {
        ...
        Iterator<WeakReference<Activity>> activityStackIterator = activityStack
                .iterator();
    ...
    void monitorActivities()
    {
        if (activityMonitor != null)
        {
            Activity activity = activityMonitor
                    .waitForActivityWithTimeout(2000L);
    ...
    private void removeActivityFromStack(Activity activity)
    {
        ...
        while (activityStackIterator.hasNext())
        {
            Activity activityFromWeakReference = activityStackIterator.next()
                    .get();
        ...
        else
        {
            for (int i = 0; i < activitiesOpened.size(); i++)
            {
                Log.d(LOG_TAG, "Activity priorly opened: "
                        + activitiesOpened.get(i).getClass().getSimpleName());
            }
            Assert.fail("No Activity named: '" + name
                    + "' has been priorly opened");
        }
    }
    ...
    private static final class RegisterActivitiesThread extends Thread
    {
        ...
        private boolean shouldMonitor()
        {
            ...
            return activityUtils != null
                    && activityUtils.shouldRegisterActivities();
        }
