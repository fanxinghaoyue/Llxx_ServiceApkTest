...
class Waiter
{
    ...
    public Waiter(Instrumentation instrumentation, ActivityUtils activityUtils,
            ViewFetcher viewFetcher, Searcher searcher, Scroller scroller,
            Sleeper sleeper)
    {
    ...
    public boolean waitForActivity(String name, int timeout)
    {
        ...
        {
            Activity currentActivity = activityMonitor
                    .waitForActivityWithTimeout(endTime - currentTime);
    ...
    public boolean waitForActivity(Class<? extends Activity> activityClass,
            int timeout)
    {
        if (isActivityMatching(activityClass,
                activityUtils.getCurrentActivity(false, false)))
        {
            return true;
        }
        ...
        while (currentTime < endTime)
        {
            ...
            if (currentActivity != null
                    && currentActivity.getClass().equals(activityClass))
            {
                foundActivity = true;
                break;
            }
     ...
     */
    private boolean isActivityMatching(Class<? extends Activity> activityClass,
    ...
    public <T extends View> boolean waitForViews(boolean scrollMethod,
    ...
    {
        View viewToWaitFor = waitForView(view, Timeout.getLargeTimeout(), true,
                true);
    ...
    public View waitForView(View view, int timeout, boolean scroll,
            ...
            int timeout, boolean scroll)
    {
        ...
        while (true)
        {
            ...
            WebElement webElementToReturn = searcher.searchForWebElement(by,
    ...
    {
        return waitForText(TextView.class, text, expectedMinimumNumberOfMatches,
                timeout, scroll, false, true);
    }
    ...
    public <T extends TextView> T waitForText(Class<T> classToFilterBy,
            String text, int expectedMinimumNumberOfMatches, long timeout,
            boolean scroll)
    {
        return waitForText(classToFilterBy, text,
                expectedMinimumNumberOfMatches, timeout, scroll, false, true);
    }
    ...
    {
        return waitForText(TextView.class, text, expectedMinimumNumberOfMatches,
                timeout, scroll, onlyVisible, hardStoppage);
    }
    ...
    public <T extends TextView> T waitForText(Class<T> classToFilterBy,
            String text, int expectedMinimumNumberOfMatches, long timeout,
            boolean scroll, boolean onlyVisible, boolean hardStoppage)
    {
        final long endTime = SystemClock.uptimeMillis() + timeout;
        ...
        while (true)
        {
            ...
            final T textViewToReturn = searcher.searchFor(classToFilterBy, text,
            ...
            Class<T> classToFilterBy)
    {
        ...
        ArrayList<T> views = RobotiumUtils.removeInvisibleViews(
                viewFetcher.getCurrentViews(classToFilterBy, true));
        ...
        catch (IndexOutOfBoundsException exception)
        {
            ...
            if (match > 1)
            {
                Assert.fail(match + " " + classToFilterBy.getSimpleName() + "s"
                        + " are not found!");
            }
            else
            {
    ...
    private Fragment getSupportFragment(String tag, int id)
    {
        ...
        try
        {
            fragmentActivity = (FragmentActivity) activityUtils
    ...
    private android.app.Fragment getFragment(String tag, int id)
    {
        ...
        {
            if (tag == null)
                return activityUtils.getCurrentActivity().getFragmentManager()
                        .findFragmentById(id);
            else
                return activityUtils.getCurrentActivity().getFragmentManager()
                        .findFragmentByTag(tag);
