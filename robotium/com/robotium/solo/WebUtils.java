...
class WebUtils
{
    ...
    public WebUtils(Config config, Instrumentation instrumentation,
            ViewFetcher viewFetcher, Sleeper sleeper)
    {
        ...
        robotiumWebCLient = new RobotiumWebClient(instrumentation,
                webElementCreator);
    }
    ...
    private ArrayList<TextView> createAndReturnTextViewsFromWebElements(
            boolean javaScriptWasExecuted)
    {
        ...
        {
            for (WebElement webElement : webElementCreator
                    .getWebElementsFromWebViews())
            {
                if (isWebElementSufficientlyShown(webElement))
                {
                    RobotiumTextView textView = new RobotiumTextView(
                            inst.getContext(), webElement.getText(),
    ...
    public ArrayList<WebElement> getWebElements(final By by,
    ...
    private ArrayList<WebElement> getWebElements(boolean javaScriptWasExecuted,
            boolean onlySufficientlyVisbile)
    {
    ...
    private String prepareForStartOfJavascriptExecution(List<WebView> webViews)
    {
        ...
        if (currentWebChromeClient != null && !currentWebChromeClient.getClass()
                .isAssignableFrom(RobotiumWebClient.class))
        {
            originalWebChromeClient = currentWebChromeClient;
        }
        robotiumWebCLient.enableJavascriptAndSetRobotiumWebClient(webViews,
    ...
    private WebChromeClient getCurrentWebChromeClient()
    {
        ...
        Object currentWebView = viewFetcher.getFreshestView(
        ...
        if (android.os.Build.VERSION.SDK_INT >= 16)
        {
            try
            {
                currentWebView = new Reflect(currentWebView).field("mProvider")
                        .out(Object.class);
            }
        ...
        try
        {
            ...
            {
                Object mClientAdapter = new Reflect(currentWebView)
                        .field("mContentsClientAdapter").out(Object.class);
                currentWebChromeClient = new Reflect(mClientAdapter)
                        .field("mWebChromeClient").out(WebChromeClient.class);
            }
            else
            {
                Object mCallbackProxy = new Reflect(currentWebView)
                        .field("mCallbackProxy").out(Object.class);
                currentWebChromeClient = new Reflect(mCallbackProxy)
                        .field("mWebChromeClient").out(WebChromeClient.class);
            }
        }
    ...
    public void enterTextIntoWebElement(final By by, final String text)
    {
        ...
        {
            executeJavaScriptFunction("enterTextById(\"" + by.getValue()
                    + "\", \"" + text + "\");");
        }
        else if (by instanceof By.Xpath)
        {
            executeJavaScriptFunction("enterTextByXpath(\"" + by.getValue()
                    + "\", \"" + text + "\");");
        }
        else if (by instanceof By.CssSelector)
        {
            executeJavaScriptFunction("enterTextByCssSelector(\""
                    + by.getValue() + "\", \"" + text + "\");");
        }
        else if (by instanceof By.Name)
        {
            executeJavaScriptFunction("enterTextByName(\"" + by.getValue()
                    + "\", \"" + text + "\");");
        }
        else if (by instanceof By.ClassName)
        {
            executeJavaScriptFunction("enterTextByClassName(\"" + by.getValue()
                    + "\", \"" + text + "\");");
        }
        else if (by instanceof By.Text)
        {
            executeJavaScriptFunction("enterTextByTextContent(\""
                    + by.getValue() + "\", \"" + text + "\");");
        }
        else if (by instanceof By.TagName)
        {
            executeJavaScriptFunction("enterTextByTagName(\"" + by.getValue()
                    + "\", \"" + text + "\");");
        }
    }
    ...
    {
        List<WebView> webViews = viewFetcher.getCurrentViews(WebView.class,
                true);
        final WebView webView = viewFetcher
                .getFreshestView((ArrayList<WebView>) webViews);
        ...
        final String javaScript = setWebFrame(
                prepareForStartOfJavascriptExecution(webViews));
    ...
    {
        final WebView webView = viewFetcher.getFreshestView(
                viewFetcher.getCurrentViews(WebView.class, true));
