...
class Clicker
{
    ...
    public Clicker(ActivityUtils activityUtils, ViewFetcher viewFetcher,
            Sender sender, Instrumentation inst, Sleeper sleeper, Waiter waiter,
            WebUtils webUtils, DialogUtils dialogUtils)
    {
    ...
    public void clickLongOnScreen(float x, float y, int time, View view)
    {
        ...
        if (time > 0)
            ...
            sleeper.sleep(
                    (int) (ViewConfiguration.getLongPressTimeout() * 2.5f));
    ...
    public void clickOnMenuItem(String text, boolean subMenu)
    {
        ...
        boolean textShown = waiter.waitForText(text, 1, WAIT_TIME,
                true) != null;

        if (subMenu && (viewFetcher.getCurrentViews(TextView.class, true)
                .size() > 5) && !textShown)
        {
    ...
    public void clickOnActionBarHomeButton()
    {
        ...
        {
            Class<?> cls = Class
                    .forName("com.android.internal.view.menu.ActionMenuItem");
        ...
        if (homeMenuItem != null)
        {
            try
            {
                activity.getWindow().getCallback().onMenuItemSelected(
                        Window.FEATURE_OPTIONS_PANEL, homeMenuItem);
            }
            catch (Exception ignored)
            {
            }
            ...
            boolean useJavaScriptToClick)
    {
        ...
        {
            webElement = waiter.waitForWebElement(by, match,
                    Timeout.getSmallTimeout(), false);
            if (webElement == null)
            {
        ...
        WebElement webElementToClick = waiter.waitForWebElement(by, match,
                Timeout.getSmallTimeout(), scroll);
        ...
        clickOnScreen(webElementToClick.getLocationX(),
                webElementToClick.getLocationY(), null);
    }
    ...
    public void clickOnText(String regex, boolean longClick, int match,
            boolean scroll, int time)
    {
        TextView textToClick = waiter.waitForText(regex, match,
                Timeout.getSmallTimeout(), scroll, true, false);
        ...
        else
        {
            ...
            if (match > 1)
            {
                Assert.fail(match + " matches of text string: '" + regex
                        + "' are not found!");
            }

            else
            {
                ArrayList<TextView> allTextViews = RobotiumUtils
                        .removeInvisibleViews(viewFetcher
                                .getCurrentViews(TextView.class, true));
                allTextViews.addAll((Collection<? extends TextView>) webUtils
                        .getTextViewsFromWebView());

                for (TextView textView : allTextViews)
                {
                    Log.d(LOG_TAG, "'" + regex + "' not found. Have found: '"
                            + textView.getText() + "'");
                }
                allTextViews = null;
    ...
    public <T extends TextView> void clickOn(Class<T> viewClass,
            String nameRegex)
    {
        T viewToClick = (T) waiter.waitForText(viewClass, nameRegex, 0,
                Timeout.getSmallTimeout(), true, true, false);
        ...
        else
        {
            ArrayList<T> allTextViews = RobotiumUtils.removeInvisibleViews(
                    viewFetcher.getCurrentViews(viewClass, true));

            for (T view : allTextViews)
            {
                Log.d(LOG_TAG, "'" + nameRegex + "' not found. Have found: '"
                        + view.getText() + "'");
            }
            Assert.fail(viewClass.getSimpleName() + " with text: '" + nameRegex
                    + "' is not found!");
        }
    }
    ...
    public ArrayList<TextView> clickInList(int line, int index, int id,
            boolean longClick, int time)
    {
        ...
        final AbsListView absListView = waiter.waitForAndGetView(index,
                AbsListView.class);
    ...
    public ArrayList<TextView> clickInRecyclerView(int itemIndex,
            int recyclerViewIndex, int id, boolean longClick, int time)
    {
        ...
        else
        {
            ...
            viewOnLine = getViewOnRecyclerItemIndex((ViewGroup) recyclerView,
                    recyclerViewIndex, itemIndex);
        }
    ...
    private void failIfIndexHigherThenChildCount(ViewGroup viewGroup, int index,
            long endTime)
    {
        while (index > viewGroup.getChildCount())
        {
            ...
            if (timedOut)
            {
                ...
                Assert.fail("Can not click on index " + index
                        + " as there are only " + numberOfIndexes
                        + " indexes available");
            }
            sleeper.sleep();
        }
    ...
    private View getViewOnAbsListLine(AbsListView absListView, int index,
            int lineIndex)
    {
        final long endTime = SystemClock.uptimeMillis()
                + Timeout.getSmallTimeout();
        ...
        while (view == null)
        {
            ...
            sleeper.sleep();
            absListView = (AbsListView) viewFetcher
                    .getIdenticalView(absListView);

            if (absListView == null)
            {
                absListView = waiter.waitForAndGetView(index,
                        AbsListView.class);
            }
    ...
    private View getViewOnRecyclerItemIndex(ViewGroup recyclerView,
            int recyclerViewIndex, int itemIndex)
    {
        final long endTime = SystemClock.uptimeMillis()
                + Timeout.getSmallTimeout();
        ...
        while (view == null)
        {
            ...
            sleeper.sleep();
            recyclerView = (ViewGroup) viewFetcher
                    .getIdenticalView(recyclerView);

            if (recyclerView == null)
            {
                recyclerView = (ViewGroup) viewFetcher.getRecyclerView(false,
                        recyclerViewIndex);
            }
