...
class ScreenshotTaker
{
    ...
    private static final long TIMEOUT_SCREENSHOT_MUTEX = TimeUnit.SECONDS
            .toMillis(2);
     ...
     * 
     */
    ScreenshotTaker(Config config, Instrumentation instrumentation,
            ActivityUtils activityUtils, ViewFetcher viewFetcher,
            Sleeper sleeper)
    {
        this.config = config;
     ...
     *
     */
    public void startScreenshotSequence(final String name, final int quality,
            final int frameDelay, final int maxFrames)
    {
        initScreenShotSaver();
        ...
        if (screenshotSequenceThread != null)
        {
            throw new RuntimeException(
                    "only one screenshot sequence is supported at a time");
        }

        screenshotSequenceThread = new ScreenshotSequenceThread(name, quality,
    ...
    {
        View decorView = viewFetcher
                .getRecentDecorView(viewFetcher.getWindowDecorViews());
    ...
    {
        ArrayList<GLSurfaceView> currentViews = viewFetcher
                .getCurrentViews(GLSurfaceView.class, true, decorView);
        ...
        for (GLSurfaceView glView : currentViews)
        {
            ...
            Renderer renderer = new Reflect(renderContainer).field("mRenderer")
                    .out(Renderer.class);

            if (renderer == null)
            {
                renderer = new Reflect(glView).field("mRenderer")
                        .out(Renderer.class);
            ...
            {
                GLRenderWrapper wrapper = new GLRenderWrapper(glView, renderer,
                        latch);
    ...
    private class ScreenshotSequenceThread extends Thread
    {
        ...
        public ScreenshotSequenceThread(String _name, int _quality,
                int _frameDelay, int _maxFrames)
        {
            name = _name;
            quality = _quality;
        ...
        public void run()
        {
            ...
            {
                if (!keepRunning || Thread.interrupted())
                    break;
                doScreenshot();
                seqno++;
                try
        ...
        public void doScreenshot()
        {
            ...
            ScreenshotRunnable r = new ScreenshotRunnable(v, final_name,
                    quality);
    ...
    private class ScreenShotSaver extends Handler
    {
        ...
        private void saveFile(String name, Bitmap b, int quality)
        {
            ...
            try
            {
                ...
                if (config.screenshotFileType == ScreenshotFileType.JPEG)
                {
                    if (b.compress(Bitmap.CompressFormat.JPEG, quality,
                            fos) == false)
                    {
                ...
                {
                    if (b.compress(Bitmap.CompressFormat.PNG, quality,
                            fos) == false)
                    {
                        Log.d(LOG_TAG, "Compress/Write failed");
                    }
